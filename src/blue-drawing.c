...
int main(int argc, char** argv)
{
  ...
  d = drawing_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  d = blue_drawing_parameter_handler(argc, argv, d);

  if (blue_drawing_check(d))
  {
    blue_drawing_check_error();
      // Return RETVAL
  return -1;
  }
...
static void version(void)
{
  fprintf(stderr, "blue-drawing 0.1\n");
}

#include "blue-drawing-usage.inc"

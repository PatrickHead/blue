...
int main(int argc, char** argv)
{
  ...
  t = design_text_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  t = blue_design_text_parameter_handler(argc, argv, t);

  if (blue_design_text_check(t))
  {
    blue_design_text_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_text_to_xml_doc(t);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-design-text 0.1\n");
}

#include "blue-design-text-usage.inc"

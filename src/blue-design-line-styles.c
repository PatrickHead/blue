/*!
    @file blue-design-line-styles.c
...
int main(int argc, char** argv)
{
  ...
  lss = design_line_styles_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  lss = blue_design_line_styles_parameter_handler(argc, argv, lss);

  if (blue_design_line_styles_check(lss))
  {
    blue_design_line_styles_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_line_styles_to_xml_doc(lss);
  if (!doc)
  {
  ...
  design_line_styles_destroy(lss);
...
static void version(void)
{
  fprintf(stderr, "blue-design-line-styles 0.1\n");
}

#include "blue-design-line-styles-usage.inc"

...
int main(int argc, char** argv)
{
  ...
  l = design_layer_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  l = blue_design_layer_parameter_handler(argc, argv, l);

  if (blue_design_layer_check(l))
  {
    blue_design_layer_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_layer_to_xml_doc(l);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-design-layer 0.1\n");
}

#include "blue-design-layer-usage.inc"

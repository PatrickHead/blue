...
int main(int argc, char** argv)
{
  ...
  r = design_radial_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  r = blue_design_radial_parameter_handler(argc, argv, r);

  if (blue_design_radial_check(r))
  {
    blue_design_radial_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_radial_to_xml_doc(r);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-design-radial 0.1\n");
}

#include "blue-design-radial-usage.inc"

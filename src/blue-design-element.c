...
int main(int argc, char** argv)
{
  ...
  e = design_element_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  e = blue_design_element_parameter_handler(argc, argv, e);

  if (blue_design_element_check(e))
  {
    blue_design_element_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_element_to_xml_doc(e);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-design-element 0.1\n");
}

#include "blue-design-element-usage.inc"

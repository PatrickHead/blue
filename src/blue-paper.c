/*!
    ...
    @timestamp Mon, 22 Dec 2014 03:24:49 +0000
...
int main(int argc, char** argv)
{
  ...
  p = paper_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  p = blue_paper_parameter_handler(argc, argv, p);

  if (blue_paper_check(p))
  {
    blue_paper_check_error();
      // Return RETVAL
  return -1;
  }

  doc = paper_to_xml_doc(p);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-paper 0.1\n");
}

#include "blue-paper-usage.inc"

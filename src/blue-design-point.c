...
int main(int argc, char** argv)
{
  ...
  p = design_point_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  p = blue_design_point_parameter_handler(argc, argv, p);

  if (blue_design_point_check(p))
  {
    blue_design_point_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_point_to_xml_doc(p);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-design-point 0.1\n");
}

#include "blue-design-point-usage.inc"

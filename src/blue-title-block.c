...
int main(int argc, char** argv)
{
  ...
  tb = title_block_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  tb = blue_title_block_parameter_handler(argc, argv, tb);

  if (blue_title_block_check(tb))
  {
    blue_title_block_check_error();
      // Return RETVAL
  return -1;
  }

  doc = title_block_to_xml_doc(tb);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-title-block 0.1\n");
}

#include "blue-title-block-usage.inc"

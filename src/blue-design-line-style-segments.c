/*!
    @file blue-design-line-style-segments.c
...
int main(int argc, char** argv)
{
  ...
  lsss = design_line_style_segments_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  lsss = blue_design_line_style_segments_parameter_handler(argc, argv, lsss);

  if (blue_design_line_style_segments_check(lsss))
  {
    blue_design_line_style_segments_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_line_style_segments_to_xml_doc(lsss);
  if (!doc)
  {
  ...
  design_line_style_segments_destroy(lsss);
...
static void version(void)
{
  fprintf(stderr, "blue-line-style-segments 0.1\n");
}

#include "blue-design-line-style-segments-usage.inc"

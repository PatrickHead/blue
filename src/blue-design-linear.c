...
int main(int argc, char** argv)
{
  ...
  l = design_linear_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  l = blue_design_linear_parameter_handler(argc, argv, l);

  if (blue_design_linear_check(l))
  {
    blue_design_linear_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_linear_to_xml_doc(l);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-design-linear 0.1\n");
}

#include "blue-design-linear-usage.inc"

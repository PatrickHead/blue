/*!
    @file blue-design-fill-styles.c
...
int main(int argc, char** argv)
{
  ...
  fss = design_fill_styles_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command fill options

  fss = blue_design_fill_styles_parameter_handler(argc, argv, fss);

  if (blue_design_fill_styles_check(fss))
  {
    blue_design_fill_styles_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_fill_styles_to_xml_doc(fss);
  if (!doc)
  {
  ...
  design_fill_styles_destroy(fss);
...
static void version(void)
{
  fprintf(stderr, "blue-design-fill-styles 0.1\n");
}

#include "blue-design-fill-styles-usage.inc"

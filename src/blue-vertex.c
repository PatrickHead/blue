...
int main(int argc, char** argv)
{
  ...
  v = vertex_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  v = blue_vertex_parameter_handler(argc, argv, v);

  if (blue_vertex_check(v))
  {
    blue_vertex_check_error();
    free(in_file_name);
      // Return RETVAL
  return -1;
  }

  doc = vertex_to_xml_doc(v);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-vertex 0.1\n");
}

#include "blue-vertex-usage.inc"

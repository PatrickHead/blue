...
int main(int argc, char** argv)
{
  ...
  m = margins_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  m = blue_margins_parameter_handler(argc, argv, m);

  if (blue_margins_check(m))
  {
    blue_margins_check_error();
    free(in_file_name);
      // Return RETVAL
  return -1;
  }

  doc = margins_to_xml_doc(m);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-margins 0.1\n");
}

#include "blue-margins-usage.inc"

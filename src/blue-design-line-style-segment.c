/*!
    @file blue-design-line-style-segment.c
...
int main(int argc, char** argv)
{
  ...
  lss = design_line_style_segment_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  lss = blue_design_line_style_segment_parameter_handler(argc, argv, lss);

  if (blue_design_line_style_segment_check(lss))
  {
    blue_design_line_style_segment_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_line_style_segment_to_xml_doc(lss);
  if (!doc)
  {
  ...
  design_line_style_segment_destroy(lss);
...
static void version(void)
{
  fprintf(stderr, "blue-design-line-style-segment 0.1\n");
}

#include "blue-design-line-style-segment-usage.inc"

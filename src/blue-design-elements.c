...
int main(int argc, char** argv)
{
  ...
  es = design_elements_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  es = blue_design_elements_parameter_handler(argc, argv, es);
...
static void version(void)
{
  fprintf(stderr, "blue-design-elements 0.1\n");
}

#include "blue-design-elements-usage.inc"

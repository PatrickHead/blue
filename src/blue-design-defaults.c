...
int main(int argc, char** argv)
{
  ...
  d = design_defaults_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  d = blue_design_defaults_parameter_handler(argc, argv, d);

  if (blue_design_defaults_check(d))
  {
    blue_design_defaults_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_defaults_to_xml_doc(d);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-design-defaults 0.1\n");
}

#include "blue-design-defaults-usage.inc"

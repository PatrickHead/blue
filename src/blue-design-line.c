...
int main(int argc, char** argv)
{
  ...
  l = design_line_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  l = blue_design_line_parameter_handler(argc, argv, l);

  if (blue_design_line_check(l))
  {
    blue_design_line_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_line_to_xml_doc(l);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-design-line 0.1\n");
}

#include "blue-design-line-usage.inc"

...
int main(int argc, char** argv)
{
  ...
  vs = vertices_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  vs = blue_vertices_parameter_handler(argc, argv, vs);

  if (blue_vertices_check(vs))
  {
    blue_vertices_check_error();
      // Return RETVAL
  return -1;
  }

  doc = vertices_to_xml_doc(vs);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-vertices 0.1\n");
}

#include "blue-vertices-usage.inc"

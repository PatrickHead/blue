...
int main(int argc, char** argv)
{
  ...
  d = design_sieve(infile, outfile);

  if (infile != stdin) fclose(infile);

    // Process specific command line options

  d = blue_design_parameter_handler(argc, argv, d);

  if (blue_design_check(d))
  {
    blue_design_check_error();
      // Return RETVAL
  return -1;
  }

  doc = design_to_xml_doc(d);
  if (!doc)
  {
...
static void version(void)
{
  fprintf(stderr, "blue-design 0.1\n");
}

#include "blue-design-usage.inc"
